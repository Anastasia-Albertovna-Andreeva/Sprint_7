...
class TestCourierLogin:
    ...
    @allure.title('Проверка успешной авторизации курьера при вводевалидных данных')
    @allure.description('проверка кода и тела ответа при успешной авторизации каурьера')
    def test_courier_login_success(self):
        response = requests.post(Urls.URL_courier_login, data=Data.valid_courier_data)
        assert response.status_code == 200 and 'id' in response.text

    @allure.title('Проверка получения ошибки при авторизации курьера при вводе невалидных данных')
    @allure.description('при тестировании передаются наборы данных с невалидным логином и паролем и проверяется код и тело ответа.')
    ...
    @allure.title('Проверка получения ошибки при авторизации курьера с пустым полем логина или пароля')
